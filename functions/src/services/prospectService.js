...
export async function envoyerMessageProspect(uid, texte) {
  ...
  // Si l'IA a extrait des infos sur le projet ce tour-ci, on les enregistre
  // (l'IA renvoie toujours l'ensemble cumulé, donc un simple remplacement suffit)
  if (result.infosExtraites) {
    miseAJour.infosCollectees = result.infosExtraites
  }
  await updateDoc(ref, miseAJour)
...
// Marquer un prospect comme traité / pas encore traité (suivi admin, voir AdminProspects.jsx)
